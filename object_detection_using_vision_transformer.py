...
class PatchEncoder(layers.Layer):
    if use_org_style:
        ...
        def __init__(self, num_patches, projection_dim, **kwargs):
            #print('>>> PatchEncoder():kwargs',kwargs)
            #>>> PatchEncoder():kwargs {
            # 'name': 'patch_encoder', 
            # 'trainable': True, 
            # 'dtype': 'float32', 
            ...
            if kwargs:
                self.name=kwargs['name']
            #print('>>> PatchEncoder():name:',self.name)
...
def run_experiment(model, learning_rate, weight_decay, batch_size, num_epochs):
    ...
    checkpoint_callback2 = keras.callbacks.ModelCheckpoint(
        "vit_object_detector_best.keras",
        monitor="val_loss",
        save_best_only=True,
        #save_weights_only=True,
    )
...
if __name__ == "__main__":
    ...
    if os.path.exists(path_images) == False or os.path.exists(path_annot) == False:
        ...
        # /tmp/.keras/datasets/caltech_101_zipped

        download_base_dir = os.path.dirname(path_to_downloaded_file)

        #print("download_base_dir:", download_base_dir)
        # /tmp/.keras/datasets
